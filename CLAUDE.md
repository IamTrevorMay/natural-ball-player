...
- `training_slots` — coach availability slots. `repeat_weekly` + `repeat_end_date` for recurrence (NOT `is_recurring`). Virtual occurrences expanded client-side; first visible occurrence per master marked non-virtual for drag. `CreateSlotPanel` supports edit mode via `existingSlot` prop. `is_exception boolean` + `original_date date` support per-occurrence exceptions (unique on `(recurrence_parent_id, original_date)`): a child row with `is_exception=true` hides that one date (tombstone, #279); `is_exception=false` means that occurrence was MOVED (#292) and renders as a real session at the child row's own `slot_date`, with that date's `slot_reservations` repointed at the child id. `buildSlotExceptionMap`/`getSlotDateException`/`collectMovedSlots` (`scheduleUtils.js`) are the shared helpers — every `training_slots` expander uses them: `fetchCoachSlots`, `fetchMyScheduleEvents`, `fetchStaffSchedule` (Schedule.js), `fetchTrainingSessions` (App.js), `fetchSlots` (CoachTools.js), `fetchData` (WorkSchedule.js). Any NEW reader must: skip any expanded series date with ANY exception row (skipping only tombstones double-renders moved dates), render moved child rows as real sessions on their own `slot_date`, and never render tombstones
...
- `users.show_on_facility_schedule boolean default true` (#260) — controls whether a coach appears as a Staff Schedule row in the facility calendar `LaneView` (Schedule.js). Role/permissions are untouched; it's a UI visibility flag. Staff toggle it via the "Manage coaches" button in the Staff Schedule band → `toggleCoachSchedule`. Rows render `visibleCoaches` (flag !== false); the manage modal lists all coaches
...
- `RecurrenceDecisionModal` offers deletion modes: one/future/all (with `allowOne` prop, default true; #279 gave slots a per-occurrence exception model so they now offer "this one" too)
