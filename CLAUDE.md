...
- `training_slots` — coach availability slots. `repeat_weekly` + `repeat_end_date` for recurrence (NOT `is_recurring`). Virtual occurrences expanded client-side; first visible occurrence per master marked non-virtual for drag. `CreateSlotPanel` supports edit mode via `existingSlot` prop. `is_exception boolean` + `original_date date` (#279) support per-occurrence tombstones: a child row with `recurrence_parent_id` set + `is_exception=true` hides that one date (unique on `(recurrence_parent_id, original_date)`); `is_exception=false` would mean a modified (not deleted) occurrence, a feature not yet built. `buildSlotExceptionMap`/`isSlotDateCancelled` (`scheduleUtils.js`) are the shared helpers — every `training_slots` expander uses them: `fetchCoachSlots`, `fetchMyScheduleEvents`, `fetchStaffSchedule` (Schedule.js), `fetchTrainingSessions` (App.js), `fetchSlots` (CoachTools.js), `fetchData` (WorkSchedule.js). Any NEW reader that expands recurring slots must do the same — never render a child row as its own session, and skip any date the map tombstones
